...
- Native Basic Settings line type edits use the official fixed 292-byte table for `LTYPE_02..09`, `LTYPE_R1..R5`, and `LTYPE_L1..L4`. Named pattern, dot-count, amplitude, and pitch fields are range-checked before source-splicing. `LTYPE_HC` and the post-table diagnostic remain read-only and are not inferred as JWW fields.
...
- Jw_cad 6.20 in a dedicated VM opened, separately saved, and reloaded generated v600 DIMENSION, BLOCK/INSERT, and external IMAGE-reference fixtures. A separate generated DIMENSION-fixture cycle added one LINE, saved a non-empty parser-clean v600 file, isolated the intended LINE from old-runtime normalization in semantic diff, and visibly reloaded the output. Public third-party v351 DIMENSION and v214 BLOCK/IMAGE samples were also converted by Jw_cad 6.20 to non-empty parser-clean v600 files and reopened; the IMAGE absolute-path reference frame, but not relocated bitmap pixels, was visible. These are generated-fixture and legacy-source conversion results, not independently distributed native-v600 DIMENSION/BLOCK/IMAGE conformance.
- Open items are classified by their actual evidence boundary, including independent-v600-samples, audit-only, metadata-ready, separate-project, and out-of-scope-for-conversion, so unresolved research items are not treated as current-version parser failures.
