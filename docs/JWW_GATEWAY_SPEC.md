...
- 公式形式どおり画面色 0（背景）、1～8（線色）、9（グレー）を読み取る
- 直後の印刷色 0～9、線幅、実点半径を読み取る
- 連続する公式240-byte範囲、画面80-byte範囲、印刷160-byte範囲を診断に表示する
- 旧推定の画面色10は印刷背景色の誤読だったため公開しない
- 直接RGB指定色を保持する
- SXF系線色番号を既知の色へ解決する
...
- 画面色・印刷色のnative編集は公式240-byte範囲を完全検証できた場合だけ許可し、範囲外は書き換えません。
- 特殊な操作表示色の候補とJWF専用値は、このnative色表へ自動適用しません。
...
Gateway maps the official fixed 292-byte line type table immediately after the verified color tables. Its structured rows are `LTYPE_02..09`, `LTYPE_R1..R5`, and `LTYPE_L1..L4`; the verified span has stable native target `jww:line-type-settings` and supports source-splice editing of named pattern, dot-count, amplitude, and pitch fields. The earlier plausible-table search remains read-only compatibility data when the official source boundary is unavailable. Public JWF references define `LTYPE_HC` as six operation/display fields. Controlled Jw_cad 10.02.1 single-change Save As tests prove that they are not serialized into JWW. The 24 bytes immediately after `LTYPE_L4` do not track those six fields and are exposed only as the neutral `meta.jwwEnvironment.lineTypes.postLineTypeTailCandidate` diagnostic value.
