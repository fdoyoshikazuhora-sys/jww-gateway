...
Line Types uses the official fixed 292-byte layout immediately after the color tables: ordinary `LTYPE_02..09`, random `LTYPE_R1..R5`, and doubled `LTYPE_L1..L4`. Patterns use exactly eight hexadecimal digits. Ordinary and doubled rows accept unit-dot count `1-32`, screen pitch `1-16`, and print pitch `1-160`; random rows accept screen/print amplitude `1-16`, screen pitch `1-16`, and print pitch `1-160`. Save As replaces only this verified span. `LTYPE_HC` remains a JWF-only operation/display setting, and the neutral 24-byte post-table diagnostic is not writable.
